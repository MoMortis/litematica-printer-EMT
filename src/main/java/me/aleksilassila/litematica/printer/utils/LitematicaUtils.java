...
public class LitematicaUtils {
     ...
     *
     * <p>实际查询与缓存由 {@link SchematicStateCache} 提供：subregion 盒索引与放置变换
     * 预计算 + 按位置缓存，原理图变化（placement 指纹 / 保底 TTL）时自动失效。
     */
    public static BlockState getSchematicBlockState(BlockPos pos) {
        if (pos == null) {
            return null;
        }
        return SchematicStateCache.INSTANCE.getSchematicState(pos);
    }
