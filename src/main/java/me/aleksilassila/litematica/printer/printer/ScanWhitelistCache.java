 ...
 * 双实例：<b>打印白名单</b>（PRINT，只管打印过滤）与<b>寻路目标白名单</b>（WALK，
 * 只筛扫描自动寻路的"待放方块"目标），两者互不干扰、配置独立。
 * 开启且列表非空时，待放方块目标只取列表内方块（匹配格式同 PRINT_SKIP 名单，
 ...
 *
 * <p>WALK 只决定"待放方块走去哪"：<b>不影响扫描本身</b>（扫描始终枚举全部方块），
 * 也不约束「寻路错误方块」「寻路多余方块」——它们各由自己的开关放行。
 */
